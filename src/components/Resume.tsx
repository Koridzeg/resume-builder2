...
const Resume: React.FC<ResumeProps> = ({ formData }) => {
    ...
    return (
        <Box display='flex' height='100%' gap='1.5rem' flexDirection='column' paddingLeft='6em' paddingTop='3.5em' sx={{ whiteSpace: 'pre-wrap' }} maxWidth='45%' >
            {formData.image && (
                <img src={formData.image} alt="" style={{ width: '246px', height: '246px', position: 'absolute', left: '82%', borderRadius: '133px' }} />
            )}
            <Box display='flex' gap='1.5em' maxWidth='100%' flexDirection='row'>
                <Typography color='#F93B1D' fontWeight='bold' fontSize='36px' maxWidth='60%' style={{ wordWrap: "break-word" }}>{formData.name}</Typography>
                <Typography color='#F93B1D' fontWeight='700' fontSize='36px' maxWidth='60%' style={{ wordWrap: "break-word" }}>{formData.surname}</Typography>
            </Box>
            ...
            {formData.experiences.some(experience => experience.position) ? (
            ...
            {formData.experiences.some(exp => exp.description) ? (<img src={smallLine} alt="" style={{ width: '90%' }} />) : null}
            {formData.educations.some(edu => edu.institute) ? (<Typography fontWeight='700' fontSize='28px' color='#F93B1D'>განათლება</Typography>) : null}
